...
**Test coverage**: no `TESTING.md` yet (greenfield). Same test-type-per-layer convention as `website.md`: components → unit, pages/data-fetching → integration, scaffolding → none. **`Gate: quick`/`Gate: full` below are defined in `ARCHITECTURE.md` §8.3's Gate legend** — every task's tests are written first (RED → GREEN → REFACTOR) and run exclusively via `make test-admin`/`make lint-admin`/`make build-admin`, never a bare `npm` invocation on the host.
