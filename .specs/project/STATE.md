...
**Current Work:** `qor-admin` AT1–AT10 (Phase 1 foundation + Phase 2 Corona design-system component library, per `.specs/tasks/admin.md`) merged (PR #3) — see AD-011. `qor-api`'s admin session/profile gap-fill (`GET /me`, `/venues/me`, `/promoters/me`) merged (PR #16) — see AD-012; this session is now proceeding into `qor-admin` AT11–AT23 (hooks/pages/layout/E2E) on branch `feat/admin-hooks-pages-mvp-core`, not yet merged. Monetization milestone (ROADMAP.md milestone 3) P1 scope (MON-01–MON-18) merged in `qor-api` (PR #14) — see AD-009. MVP Core (PRs #3–#5, #7, #8) and the entire Social & Notifications milestone (PRs #10–#13) are also fully merged in `api` — see AD-008. `mobile`/`website`/`landingpage` submodules still have no feature work; Monetization P2 (MON-19–26) is deferred; `qor-admin` AT24–AT34 (Monetization UI) is deferred to a follow-up session after AT11–AT23 lands.
