...
**Current Work:** `qor-website` now has its first code — MVP Core Phase 1 (W1–W10 of `.specs/tasks/website.md`: scaffold, Docker/CI wiring, typed `/api/v1` client, enum mirrors, and the full NIGHTLIFE-GV design-system component library) merged (PR #1, see AD-015), mirroring the same foundation-first split `qor-admin` used for its own MVP Core. W11–W24 (data hooks, pages, layout/nav, E2E) are a follow-up session. `qor-admin` Monetization UI (AT24–AT34) is merged (PR #5, see AD-014) — Plan CRUD for Super Admin, the organizer plan/usage view, and the publish-quota at-limit gate on event submission are all E2E-verified working in a real browser (`make e2e-admin`), alongside the already-merged MVP Core (AT1–AT23, PRs #3–#4). `qor-admin` now covers every `.specs/tasks/admin.md` task through Milestone 3; P2 (change-plan/upgrade/downgrade/cancel, MON-19–26) remains deferred — no `qor-api` route exists for it yet. This required two `qor-api` fixes during MVP Core: an admin session/profile gap-fill (`GET /me`, `/venues/me`, `/promoters/me`, PR #16, see AD-012) and a critical fix making the admin guard's Sanctum SPA cookie auth actually work in a real browser for the first time (PR #18, see AD-013) — both merged. Monetization milestone (ROADMAP.md milestone 3) P1 scope (MON-01–MON-18) merged in `qor-api` (PR #14) — see AD-009. MVP Core `api` (PRs #3–#5, #7, #8) and the entire Social & Notifications milestone (PRs #10–#13) are also fully merged — see AD-008. `mobile`/`landingpage` submodules still have no feature work.
...
- [ ] `mobile`, `landingpage` submodules have no feature work yet (README-only) — next up per ROADMAP.md. (`admin` and `website` now do — see AD-011–AD-015.)
...
- [x] `qor-website` W1–W10 (foundation + NIGHTLIFE-GV design-system library) — merged (PR #1, see AD-015) 2026-09-02, `review-react-web` findings addressed. W11–W24 (data hooks, pages, layout wiring, E2E) next for `qor-website`.
