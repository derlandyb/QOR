...
**Current Work:** `qor-admin` AT1–AT10 (Phase 1 foundation + Phase 2 Corona design-system component library, per `.specs/tasks/admin.md`) merged (PR #3) — see AD-011. `qor-api`'s admin session/profile gap-fill (`GET /me`, `/venues/me`, `/promoters/me`, PR #16) and a critical fix making the admin guard's Sanctum SPA cookie auth actually work in a real browser for the first time (PR #18) are both merged — see AD-012/AD-013. `qor-admin` AT11–AT23 (hooks/pages/layout/E2E) is complete on branch `feat/admin-hooks-pages-mvp-core`, E2E-verified end-to-end, not yet merged/PR'd — that's next. Monetization milestone (ROADMAP.md milestone 3) P1 scope (MON-01–MON-18) merged in `qor-api` (PR #14) — see AD-009. MVP Core (PRs #3–#5, #7, #8) and the entire Social & Notifications milestone (PRs #10–#13) are also fully merged in `api` — see AD-008. `mobile`/`website`/`landingpage` submodules still have no feature work; Monetization P2 (MON-19–26) is deferred; `qor-admin` AT24–AT34 (Monetization UI) is deferred to a follow-up session after AT11–AT23's PR merges.
...
- [x] `qor-admin` AT11–AT23 (hooks, pages, layout wiring, E2E) complete and E2E-verified on branch `feat/admin-hooks-pages-mvp-core` 2026-09-01 (see AD-013's api-side prerequisite fixes) — PR not yet opened/merged, next step for a follow-up session.
